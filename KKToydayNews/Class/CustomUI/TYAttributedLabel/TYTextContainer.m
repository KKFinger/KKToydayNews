...
- (void)saveTextStorageRectWithFrame:(CTFrameRef)frame
{
    ...
    CGFloat viewWidth = _attriTextWidth;
...
- (CGSize)getSuggestedSizeWithFramesetter:(CTFramesetterRef)framesetter width:(CGFloat)width
{
    ...
    if (_attriTextHeight > 0) {
        return CGSizeMake(_attriTextWidth > 0 ? _attriTextWidth : width, _attriTextHeight);
    }
...
- (instancetype)createTextContainerWithContentSize:(CGSize)contentSize
{
    ...
    _attriTextWidth = size.width;
    _attriTextHeight = size.height;
    
    // 创建CTFrameRef
    _frameRef = [self createFrameRefWithFramesetter:framesetter textSize:CGSizeMake(_attriTextWidth, contentSize.height > 0 ? contentSize.height : _attriTextHeight)];
